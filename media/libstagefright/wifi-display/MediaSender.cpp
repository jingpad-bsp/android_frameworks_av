    ...
    if (mMode == MODE_TRANSPORT_STREAM) {
        ...
        for (;;) {
            ...
            if (err == OK) {
                ...
                if(info->mIsAudio) {
                   ALOGV("wfd: MediaSender::queueAccessUnit Audio Packet size: %zu, time: %lld",tsPackets->size(), (long long)nowUs);
                } else {
                   ALOGV("wfd: MediaSender::queueAccessUnit Video Packet size: %zu, time: %lld",tsPackets->size(), (long long)nowUs);
                }
