...
status_t Converter::feedEncoderInputBuffers() {
    ...
    if(mIsVideo && mInputBufferQueue.size() > 2 && mAvailEncoderInputIndices.size() == 0) {
       dropAFrame();
       ALOGI(" mInputBufferQueue size %zu ,mAvailEncoderInputIndices size %zu ",
    ...
    for (;;) {
        ...
        if (flags & MediaCodec::BUFFER_FLAG_EOS) {
            ...
            if (flags & MediaCodec::BUFFER_FLAG_CODECCONFIG) {
                ...
                notify->post();
                if( mIsVideo && ( mFrameCount <= 2) ){
                    requestIDRFrame();
                    mFrameCount ++;
                    ALOGI("requestIDRFrame()  at mFrameCount:  %d" ,mFrameCount);
                }
            }
        }
